...
should be able to infer the underlying parameters of the model and
thereby predict individual outcomes. I’m mostly familiar with solving
this problem using Bayesian inference. Luckily, a Finnish master’s
thesis Forslund (2025) showed that this is feasible for at least one
...
procedures. The estimate is a distribution, constructed by combining the
likelihood function with prior distributions placed on the free
parameters.

Assuming that uncertainty in estimates is substantial, in my eyes
planning adaptive therapy for individual patients is a type of
sequential decision-making under uncertainty. Explore-exploit dynamics
could apply, and planning treatment might benefit from balancing reward
maximization and information gain. In this light, deciding when to treat
would also depend on the potential information gain from applying
treatment.
...
The following data are PSA (biomarker of total cell count) readings
